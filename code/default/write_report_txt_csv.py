...
def write_report_result(model_scores,model_scores_base,model_scores_CFS,model_scores_Java_Rule,model_scores_stepwise,typesmell):
    ...
    for sm in select_method:
        ...
        for x in sm[0]:
            print(sm[1])
            report = open(typesmell + '\\report\\' + x[2] + '-report_' + sm[1] + '.txt','w')
            rows_tmp = model_function.PrintReport(x,report)
    ...
    ans.to_csv(typesmell + '\\' + typesmell+'-ML-update.csv',index=False)
